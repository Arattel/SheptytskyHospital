...
class AvartanSliderLiteWPFunctions {
    ...
    public static function filterSettings($settings, $type) {
        ...
        if ($type == 'layer') {
            ...
            if ($current_version >= 1.3 && !isset($settings['animation_ease_in'])) return $settings;
    ...
    public static function FilterAdvaceStyle($settings) {
        ...
        if ($allcss) {
            $count = 0;
            foreach ($allcss as $single_css) {
                if ($single_css != '') {
                    $check_style = explode(':', $single_css);
                    $css = isset($check_style[0]) ? strtolower(trim($check_style[0])) : '';
                    if ($css == 'width') {
                        unset($allcss[$count]);
                        $px_value = self::avsConvertToPX($check_style[1]);

                        //For image
                        if ($settings['type'] == 'image' && strrpos($check_style[1], '%') !== false) {
                            echo $ori = floatval(substr(strtolower($check_style[1]), 0, -1));
                        ...
                        //For image
                        if ($settings['type'] == 'image' && strrpos($check_style[1], '%') !== false) {
                            $ori = floatval(substr(strtolower($check_style[1]), 0, -1));
    ...
    */
    public static function filterIndividualToAdvaceStyle($settings,$layer_type) {
        ...
        $advanced_style[1] = (isset($settings['line_height']['desktop']) && $settings['line_height']['desktop'] != '') ? 'line-height:'.$settings['line_height']['desktop'].'px' : '';
