...
jQuery(window).load(function () {
    ...
    jQuery('#sbtDeactivationFormavl').click(function(event){
        event.preventDefault();
        var selected_option_de_avl = jQuery('input[name=sol_deactivation_reasons_avl]:checked', '#frmDeactivationavl').val();
        var selected_option_de_id_avl = jQuery('input[name=sol_deactivation_reasons_avl]:checked', '#frmDeactivationavl').attr("id");
        var selected_option_de_text_avl = jQuery("label[for='"+selected_option_de_id_avl+"']").text();
        jQuery.ajax({
            url: ajaxurl,
            method: 'POST',
            data: {
                'action': 'avl_sbtDeactivationform',
                'deactivation_option': selected_option_de_avl,
                'deactivation_option_text': selected_option_de_text_avl,
                'deactivation_option_other': jQuery('.sol_deactivation_reason_other_avl').val()
            },
            complete: function () {
	...
	jQuery(document).find('#TB_window').width( '700' ).height( '450' ).css( 'margin-left', - 700 / 2 );
        jQuery(document).find('#TB_ajaxContent').width( '640' ).height( '450' );
...
jQuery(document).ready(function () {
    ...
    */
     ...
     */
    delete_slider_dialog = jQuery("#dialog_delete_slider_confirm").dialog({
        resizable: false,
        draggable: false,
        height: "auto",
        width: 400,
   ...
   /*
     ...
     */
    create_new_slider_dialog = jQuery("#dialog_create_new_slider").dialog({
        resizable: false,
        draggable: false,
        width: "850px",
        modal: true,
     ...
     */
    advance_embadded_slider_dialog = jQuery("#dialog_advance_embadded_slider").dialog({
        resizable: false,
        draggable: false,
        height: "auto",
        width: 350,
    ...
    */
     ...
     */
    loader_dialog = jQuery("#dialog_loader_style").dialog({
        resizable: false,
        draggable: false,
        autoOpen: false,
        height: 410,
     ...
     */
    arrow_dialog = jQuery("#dialog_arrows_style").dialog({
        resizable: false,
        draggable: false,
        autoOpen: false,
        height: 320,
     ...
     */
    bullet_dialog = jQuery("#dialog_bullets_style").dialog({
        resizable: false,
        draggable: false,
        autoOpen: false,
        height: 250,
    ...
    */
     ...
     */
    delete_slide_dialog = jQuery("#dialog_delete_slide_confirm").dialog({
        resizable: false,
        draggable: false,
        height: "auto",
        width: 400,
     ...
     */
    preview_slide_dialog = jQuery("#dialog_preview_slide").dialog({
        resizable: false,
        draggable: false,
        autoOpen: false,
    ...
    */
    jQuery('.as-animation-effect-list li').on('click', function () {
        jQuery('.as-animation-effect-list li').removeClass('as-active');
        if(!jQuery('.as-animation-effect-list li').hasClass('as-pro-version'))
            jQuery(this).addClass('as-active');
    });
    ...
    */
    ...
    jQuery('.as-font-color-hover.as-color-picker').closest(".wp-picker-container").find(".wp-color-result").addClass("as-btn-color-picker as-font-color-btn-hover");
    jQuery('.as-background-color-hover.as-color-picker').closest(".wp-picker-container").find(".wp-color-result").addClass("as-btn-color-picker as-bgcolor-btn-hover");
    ...
    jQuery('.as-admin').on('click', '.as-editor-html5Url-search', function() {
        ...
        var ogv_url = jQuery.trim(jQuery('.as-layer-video-element').find('.as-editor-htmlWebm').val());
...
function avsSelectElement(element) {
    ...
    if(type == "video"){
        ...
        jQuery('.as-slide-layer-width,.as-slide-layer-height').live('click', function() {
            if(element.hasClass('as-active-element')) {
                avsProPopup();
            }
        });
...
function avsLayerDefVal() {
    ...
    var layer = avsGetEleObj(current_index);
    var x_position = avsGetObjVal(layer,'x_position'),
...
function avsElementsColorPicker() {
    ...
    jQuery('.as-admin .as-edit-slide .as-font-color').wpColorPicker({
        ...
        change: function (event, ui) {
            if(changeD) {
                jQuery('.as-font-color').val('#000000');
                jQuery('.as-font-color').closest('.as-form-control-group').find('.wp-color-result span').css('background', '#000000');
                avsProPopup();
            }
        },
        // a callback to fire when the input is emptied or an invalid color
